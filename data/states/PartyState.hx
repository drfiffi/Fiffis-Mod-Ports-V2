...
public var titleGroup:FlxTypedGroup;
...
public var rulesGroup:FlxTypedGroup;
...
function postCreate()
{
	...
	FlxG.resizeWindow(448, 672);
	FlxG.resizeGame(448, 672);
    Lib.current.x = 0;
    Lib.current.y = 0;
    ...
    topChecker = new FlxBackdrop(Paths.image('mpds/rules/checkerboard'), FlxAxes.XY);
    ...
    botChecker = new FlxBackdrop(Paths.image('mpds/rules/checkerboard'), FlxAxes.XY);
    ...
    new FlxTimer().start(2.5, function(t:FlxTimer)
    {
        ...
        for(e in [topFlowers, topCoinStatic, topCoinSmallLeft, topCoinSmallRight, topCoinBigLeft, topCoinBigRight, botFlowers, sideChars, mainChars])
            FlxTween.tween(e, {y: 0}, 2.671, {startDelay: 1, ease: FlxEase.smoothStepOut});
        ...
        for(animPlayShit in [topStar, topDice, topCoinSmallLeft, topCoinSmallRight, topCoinBigLeft, topCoinBigRight])
            animPlayShit.animation.play('idle', true);
                        ...
                        onComplete: function(t:FlxTween)
                        {
                            ...
                            FlxTween.tween(hiderB, {alpha: 1}, 1, {
                                onComplete: function(t:FlxTween)
                                {
                                    //whatever needs to be done here to make the song start dewott help pls
                                    // :dave:
                                        PlayState.loadSong('Last Course', 'normal', false, false);
