...
import flixel.util.FlxSpriteUtil;
	...
	var topCanvas:FlxSprite;
	var clock:FlxSprite;
	var clockHands:FlxSprite;
	var topBar:PaletteSprite;
	var blackLine:FlxSprite;
	var currFav:Int = 10;
	var nameText:PaletteText;
	var timeText:PaletteText;
	var dateText:PaletteText;
	var calendar:FlxSprite;
	var calendarTextGroup:FlxGroup;
	var oldDay:Int = -1;
	var mainCam:FlxCamera;
	var bottomScreen:FlxSprite;
	var bottomCanvas:FlxSprite;
	var palette:FlxSprite;
	var fadeOut:Array<String>;
	var fadeIn1:Array<String>;
	var fadeIn2:Array<String>;
	var moveIn:Array<String>;
	var dayIndicator:PaletteSprite;
	var separators:FlxSprite;
	var intro:DSVideo;
	var introStatic:FlxSprite;
	var broWait:FlxSprite;
	var touch:FlxText;
	var touchTimer:FlxTimer;
	var lazyAss:FlxTimer;
	var doItYourself:Bool = false;
	var activeGroup:ButtonGroup;
	var mainMenuGroup:ButtonGroup;
	var game:Button;
	var pictochat:Button;
	var exit:Button;
	var gamePak:FlxSprite;
	var brightness:Button;
	var settings:Button;
	var alarm:Button;
	var overlay:FlxSprite;
	var message:FlxSprite;
	var shutdown:FlxText;

    var state:String;
    var DSStates:Array = ['Welcome', 'BIOS', 'BIOSSettings', 'FlashCart'];
	...
	function create(){
		...
		palette = new FlxSprite(0, 0).loadGraphic(Paths.image('DSBIOS/palette'));
		...
		introStatic = new FlxSprite(0, 0).loadGraphic(Paths.image('DSBIOS/dsintrostatic'));
	...
	function fadeInAndOut(text:FlxText)
	{
	...
	// sincerely: atpx8
	function cleanLine(startX, startY, endX, endY, color, image:BitmapData):Float
	{
	...
	function clockHandFix(length, angle, color):Void
	{
	...
	function removeSpacers(text:FlxText, depth:Int = 13):Void
	{
	...
	function addLeading(text:FlxText, num:Int):Void
	{
		if (num < 10)
		{
			text.text += "0";
	...
	function spaceNum(i:Int, digs:Int):String
	{
		var ret = "";
		for (id in 0...digs)
		{
	...
	function setUpDays(group:FlxGroup, offx, offy):Void
	{
	...
	function fadeShitIn(index:Int, sprite:Dynamic)
	{
		sprite.alpha = 0;
		FlxTween.tween(sprite, {alpha: 1}, 0.25);
	}

	function hideAll(index:Int, sprite:Dynamic)
	{
		sprite.alpha = 0;
	}

	function fadeShitOut(index:Int, sprite:FlxSprite)
	{
		FlxTween.cancelTweensOf(sprite);
		FlxTween.tween(sprite, {alpha: 0}, 0.25);
	}

	function moveShitIn(index:Int, sprite:FlxSprite)
	{
		FlxTween.tween(sprite, {y: sprite.y + 16}, 0.25);
	}

	function forEachReflect(strs:Array<String>, func:String, index:Int, wipe:Bool):Void
	{
