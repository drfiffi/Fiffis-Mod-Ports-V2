...
function postCreate(){
    ...
    infoTxtAccuracy = new FlxText(-200, 695, FlxG.width, 'Accuracy: 100%', 10, true);
    infoTxtMisses = new FlxText(0, 695, FlxG.width, 'Misses: 0', 10, true);
    infoTxtCombo = new FlxText(200, 695, FlxG.width, 'Combo: 0', 10, true);
    ...
    for(i in [thetext, timeTxt, healthBarNew, healthBarBGNew, playerIcon, opponentIcon, infoTxtAccuracy, infoTxtCombo, infoTxtMisses, playerScore, dadScoreTxt]) i.camera = camOther;
}
...
function onNoteHit(_){
    if(_.player){
            for(i in [infoTxtAccuracy, infoTxtCombo, infoTxtMisses]){
            lolzZoom = FlxTween.tween(i.scale, {x: 1.1, y: 1.1}, 0.05, {ease: FlxEase.sineInOut, onComplete: function(){
                FlxTween.tween(i.scale, {x: 1, y: 1}, 0.05, {ease: FlxEase.sineInOut});
            }});
        }
    }
    if(!_.player){
